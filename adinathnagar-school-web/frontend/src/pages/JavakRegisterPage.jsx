...
import { Search, Filter, Download, Eye, School, FileText } from 'lucide-react';

const JavakRegisterPage = () => {
  ...
  const fetchJavakRegister = async () => {
    try {
      setLoading(true);
      const data = await registerAPI.getJavak();
      setStudents(data);
      setFilteredStudents(data);
      setError('');
    } catch (err) {
      console.error('Error fetching javak register:', err);
      setError('જાવક રજીસ્ટર લોડ કરવામાં ક્ષતિ આવી.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const applyFilters = () => {
    ...
    if (selectedYear !== '') {
      temp = temp.filter(s => {
        if (!s.leavingDate) return false;
        const year = new Date(s.leavingDate).getFullYear().toString();
  ...
  // Extract unique leaving years from students list
  const getUniqueYears = () => {
    const years = students
      .map(s => s.leavingDate ? new Date(s.leavingDate).getFullYear().toString() : null)
  ...
  const handleExportCSV = () => {
    ...
    const headers = [
      ...
      'Standard (ધોરણ)', 
      'Mobile (મોબાઈલ નંબર)',
      'Leaving Date (કમી તારીખ)',
      'Destination School (ગયેલ શાળા)',
      'Remarks (રીમાર્ક)'
    ];
    ...
    const rows = filteredStudents.map((s, idx) => [
      ...
      s.mobile || '',
      s.leavingDate ? new Date(s.leavingDate).toLocaleDateString('en-GB') : '',
      `"${(s.destinationSchool || '').replace(/"/g, '""')}"`,
      `"${(s.remarks || '').replace(/"/g, '""')}"`
    ]);
    ...
    link.setAttribute('download', `javak_register_${new Date().getTime()}.csv`);
  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div style={headerStyle}>
        <div>
          <h1 style={{ fontSize: '1.75rem', fontWeight: '700' }}>જાવક રજીસ્ટર (Exit Register)</h1>
          <p style={{ color: '#94a3b8', fontSize: '0.9rem' }}>શાળા છોડી ગયેલ (દાખલ કમી થયેલ) વિદ્યાર્થીઓની યાદી</p>
        </div>
        <button onClick={handleExportCSV} className="btn btn-secondary" style={{ color: '#f43f5e', borderColor: 'rgba(244, 63, 94, 0.3)' }} disabled={filteredStudents.length === 0}>
      ...
      <div className="glass-card" style={filtersCardStyle}>
        ...
        <div style={selectsContainerStyle}>
          ...
          <select
            ...
            style={{ width: '100%', flex: 1 }}
          >
            <option value="">તમામ જાવક વર્ષ</option>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h2 style={{ fontSize: '1.1rem', color: '#94a3b8' }}>
          કુલ કમી વિદ્યાર્થીઓ: <strong style={{ color: '#f43f5e' }}>{toGujaratiNumbers(filteredStudents.length)}</strong>
        </h2>
      </div>
            ...
            <tbody>
              {filteredStudents.map((student, idx) => (
                <tr key={student.id}>
                  ...
                  <td>{student.leavingDate ? new Date(student.leavingDate).toLocaleDateString('gu-IN') : '-'}</td>
                  <td>
...
export default JavakRegisterPage;
